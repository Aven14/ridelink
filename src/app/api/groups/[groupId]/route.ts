...
export async function DELETE(req: Request, { params }: { params: Promise<{ groupId: string }> }) {
  ...
  // Vérifier que l'utilisateur est bien le leader
  ...
  // Informer les membres via Pusher que le groupe est dissous
  await pusherServer.trigger(`presence-ride-${groupId}`, "group-deleted", {});

  // Supprimer les membres puis le groupe (cascade)
